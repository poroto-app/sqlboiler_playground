...
var (
	PostImageAllColumns         = postImageAllColumns
	PostImageColumnsWithDefault = postImageColumnsWithDefault
)

// GetID get ID from model object
func (o *PostImage) GetID() int {
	return o.ID
}

// GetIDs extract IDs from model objects
func (s PostImageSlice) GetIDs() []int {
...
func (s PostImageSlice) GetIntfIDs() []interface{} {
...
func (s PostImageSlice) ToIDMap() map[int]*PostImage {
	result := make(map[int]*PostImage, len(s))
	for _, o := range s {
		result[o.ID] = o
	}
	return result
...
func (s PostImageSlice) ToUniqueItems() PostImageSlice {
	result := make(PostImageSlice, 0, len(s))
...
func (s PostImageSlice) FindItemByID(id int) *PostImage {
	for _, o := range s {
		if o.ID == id {
			return o
		}
...
func (s PostImageSlice) FindMissingItemIDs(expectedIDs []int) []int {
	if len(s) == 0 {
		return expectedIDs
	}
	result := []int{}
...
func (o PostImageSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}

	nzDefaults := queries.NonZeroDefaultSet(postImageColumnsWithDefault, o[0])
	nzUniques := queries.NonZeroDefaultSet(mySQLPostImageUniqueColumns, o[0])
	if len(nzUniques) == 0 {
	...
	if !updateColumns.IsNone() && len(update) == 0 {
		return 0, errors.New("models: unable to upsert post_images, could not build update column list")
	}
	...
	if len(update) == 0 {
		fmt.Fprintf(
			buf,
			"INSERT IGNORE INTO `post_images`(%s) VALUES %s",
		...
		fmt.Fprintf(
			buf,
			"INSERT INTO `post_images`(%s) VALUES %s ON DUPLICATE KEY UPDATE ",
			strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, insert), ","),
			strmangle.Placeholders(false, len(insert)*len(o), 1, len(insert)),
		)
	...
	query := buf.String()
	valueMapping, err := queries.BindMapping(postImageType, postImageMapping, insert)
	if err != nil {
		return 0, err
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to upsert for post_images")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by upsert for post_images")
	}

	if len(postImageAfterUpsertHooks) != 0 {
		for _, obj := range o {
...
// DeleteAllByPage delete all PostImage records from the slice.
...
func (s PostImageSlice) DeleteAllByPage(ctx context.Context, exec boil.ContextExecutor, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpdateAllByPage update all PostImage records from the slice.
// This function updates data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s PostImageSlice) UpdateAllByPage(ctx context.Context, exec boil.ContextExecutor, cols M, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// InsertAllByPage insert all PostImage records from the slice.
// This function inserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s PostImageSlice) InsertAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
	...
	// MySQL max placeholders = 65535
	chunkSize := MaxPageSize / reflect.ValueOf(&PostImageColumns).Elem().NumField()
...
// UpsertAllByPage upsert all PostImage records from the slice.
// This function upserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s PostImageSlice) UpsertAllByPage(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// LoadPostsByPage performs eager loading of values by page. This is for a N-1 relationship.
func (s PostImageSlice) LoadPostsByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadPostsByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s PostImageSlice) LoadPostsByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
	for _, chunk := range chunkSlice[*PostImage](s, pageSize) {
		if err := chunk[0].L.LoadPost(ctx, e, false, &chunk, queryMods(mods)); err != nil {
			return err
		}
	}
	return nil
...
func (s PostImageSlice) GetLoadedPosts() PostSlice {
	result := make(PostSlice, 0, len(s))
	mapCheckDup := make(map[*Post]struct{})
	for _, item := range s {
		if item.R == nil || item.R.Post == nil {
			continue
		}
		if _, ok := mapCheckDup[item.R.Post]; ok {
			continue
		}
		result = append(result, item.R.Post)
		mapCheckDup[item.R.Post] = struct{}{}
	}
	return result
}
