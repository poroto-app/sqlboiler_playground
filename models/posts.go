...
type postR struct {
	...
	PostImages PostImageSlice `boil:"PostImages" json:"PostImages" toml:"PostImages" yaml:"PostImages"`
}
...
func (r *postR) GetPostImages() PostImageSlice {
	if r == nil {
		return nil
	}
	return r.PostImages
...
var (
	postAllColumns            = []string{"id", "user_id", "content", "created_at", "updated_at"}
	postColumnsWithoutDefault = []string{"user_id", "content"}
...
// PostImages retrieves all the post_image's PostImages with an executor.
func (o *Post) PostImages(mods ...qm.QueryMod) postImageQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`post_images`.`post_id`=?", o.ID),
	)
...
// LoadPostImages allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (postL) LoadPostImages(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	var slice []*Post
	var object *Post
	...
	query := NewQuery(
		qm.From(`post_images`),
		qm.WhereIn(`post_images.post_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.PostImages = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.PostID {
				local.R.PostImages = append(local.R.PostImages, foreign)
				if foreign.R == nil {
					foreign.R = &postImageR{}
				}
...
// AddPostImages adds the given related objects to the existing relationships
// of the post, optionally inserting them as new records.
// Appends related to o.R.PostImages.
// Sets related.R.Post appropriately.
func (o *Post) AddPostImages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PostImage) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.PostImages = append(o.R.PostImages, related...)
	}
...
var (
	PostAllColumns         = postAllColumns
	PostColumnsWithDefault = postColumnsWithDefault
)

// GetID get ID from model object
func (o *Post) GetID() int {
	return o.ID
}

// GetIDs extract IDs from model objects
func (s PostSlice) GetIDs() []int {
...
func (s PostSlice) GetIntfIDs() []interface{} {
...
func (s PostSlice) ToIDMap() map[int]*Post {
	result := make(map[int]*Post, len(s))
	for _, o := range s {
		result[o.ID] = o
	}
	return result
...
func (s PostSlice) ToUniqueItems() PostSlice {
...
func (s PostSlice) FindItemByID(id int) *Post {
	for _, o := range s {
		if o.ID == id {
			return o
		}
...
func (s PostSlice) FindMissingItemIDs(expectedIDs []int) []int {
	if len(s) == 0 {
		return expectedIDs
	}
	result := []int{}
...
func (o PostSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for i, row := range o {
		...
		wl, _ := columns.InsertColumnSet(
			...
			postColumnsWithDefault,
			postColumnsWithoutDefault,
			queries.NonZeroDefaultSet(postColumnsWithDefault, row),
		)
		if i == 0 {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to insert all from post slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by insertall for posts")
	}

	if len(postAfterInsertHooks) != 0 {
		for _, obj := range o {
...
func (o PostSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}

	nzDefaults := queries.NonZeroDefaultSet(postColumnsWithDefault, o[0])
	nzUniques := queries.NonZeroDefaultSet(mySQLPostUniqueColumns, o[0])
	if len(nzUniques) == 0 {
	...
	if !updateColumns.IsNone() && len(update) == 0 {
		return 0, errors.New("models: unable to upsert posts, could not build update column list")
	}
	...
	if len(update) == 0 {
		fmt.Fprintf(
			buf,
			"INSERT IGNORE INTO `posts`(%s) VALUES %s",
		...
		fmt.Fprintf(
			buf,
			"INSERT INTO `posts`(%s) VALUES %s ON DUPLICATE KEY UPDATE ",
			strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, insert), ","),
			strmangle.Placeholders(false, len(insert)*len(o), 1, len(insert)),
		)
	...
	query := buf.String()
	valueMapping, err := queries.BindMapping(postType, postMapping, insert)
	if err != nil {
		return 0, err
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to upsert for posts")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by upsert for posts")
	}

	if len(postAfterUpsertHooks) != 0 {
		for _, obj := range o {
...
// DeleteAllByPage delete all Post records from the slice.
...
func (s PostSlice) DeleteAllByPage(ctx context.Context, exec boil.ContextExecutor, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpdateAllByPage update all Post records from the slice.
// This function updates data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s PostSlice) UpdateAllByPage(ctx context.Context, exec boil.ContextExecutor, cols M, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// InsertAllByPage insert all Post records from the slice.
// This function inserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s PostSlice) InsertAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
	...
	// MySQL max placeholders = 65535
	chunkSize := MaxPageSize / reflect.ValueOf(&PostColumns).Elem().NumField()
...
// UpsertAllByPage upsert all Post records from the slice.
// This function upserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s PostSlice) UpsertAllByPage(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// LoadPostImagesByPage performs eager loading of values by page. This is for a 1-M or N-M relationship.
func (s PostSlice) LoadPostImagesByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadPostImagesByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s PostSlice) LoadPostImagesByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
	for _, chunk := range chunkSlice[*Post](s, pageSize) {
		if err := chunk[0].L.LoadPostImages(ctx, e, false, &chunk, queryMods(mods)); err != nil {
			return err
		}
	}
	return nil
...
func (s PostSlice) GetLoadedPostImages() PostImageSlice {
	result := make(PostImageSlice, 0, len(s)*2)
	for _, item := range s {
		if item.R == nil || item.R.PostImages == nil {
			continue
		}
		result = append(result, item.R.PostImages...)
	}
	return result
}

// LoadUsersByPage performs eager loading of values by page. This is for a N-1 relationship.
func (s PostSlice) LoadUsersByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadUsersByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s PostSlice) LoadUsersByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
	for _, chunk := range chunkSlice[*Post](s, pageSize) {
...
func (s PostSlice) GetLoadedUsers() UserSlice {
