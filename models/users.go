...
// LoadPostsByPage performs eager loading of values by page. This is for a 1-M or N-M relationship.
func (s UserSlice) LoadPostsByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadPostsByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s UserSlice) LoadPostsByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
	for _, chunk := range chunkSlice[*User](s, pageSize) {
		if err := chunk[0].L.LoadPosts(ctx, e, false, &chunk, queryMods(mods)); err != nil {
			return err
		}
	}
	return nil
...
func (s UserSlice) GetLoadedPosts() PostSlice {
	result := make(PostSlice, 0, len(s)*2)
	for _, item := range s {
		if item.R == nil || item.R.Posts == nil {
			continue
		}
		result = append(result, item.R.Posts...)
	}
	return result
}
